...
import com.company.creational.simpleFactory.Payment;
import com.company.creational.simpleFactory.PaymentFactory;
import com.company.creational.simpleFactory.TypePayment;
import com.company.creational.prototype.card.PrototypeCard;
import com.company.creational.prototype.card.PrototypeFactory;
...
import static com.company.creational.prototype.card.PrototypeFactory.CartType.AMEX;
import static com.company.creational.prototype.card.PrototypeFactory.CartType.VISA;
...
//        probarDecorator();
    ...
    // Ocultar la complejidad de nuestro sistema (y componentes subsistema)
    ...
    // Añadir funcionalidades sin modificar su interfaz, a objetos ya existentes
    // Podemos obtener combinaciones de comportamientos o si deseamos eliminar alguno
    // Es una forma de agregar envoltorios
    // Nos obliga a encadenar objetos
    ...
    // Se usa con el fin de desacoplar las abstracciones de las implementaciones
    // De modo que se pueden hacer cambios en las estructuras sin afectar una a la otra,
    // cambios en la abstraccion y cambios en la implementacion
    // Extender jerarquias entre abstractor e implementador, numerosas clases
    // Construir dos jerarquías separadas y crear un puente
    // El imlementor esta presente dentro de la Abstracción
    ...
    // Nos permite acoplar dos interfacez incompatibles
    // Puede dificultar la legibilidad
    // Cuando se nos imponen cambios difíciles de aplicar
    // Rapper o envoltorio, permite una interfaz en otra

    //Tambien se puede usar un ObjectAdapter, que usa composition en lugar de implementar
    ...
    private static void probarSinglenton(){
        ...
        System.out.println(com.company.creational.singlenton.Card.getINSTANCE());

        com.company.creational.singlenton.Card card = com.company.creational.singlenton.Card.getINSTANCE();
        System.out.println(card);
    }

    /** Clonar objetos hace mas eficiente la creación de objetos cuando esto requiere y consume bastantes recursos */
    ...
    private static void probarAbstractFactory() {
        ...
        AbstractFactory abstractFactory2 = FactoryProvider.getFactory("PayScheme");
        PaymentScheme scheme = (PaymentScheme) abstractFactory2.create("Cash");

        System.out.println("Un tarjeta de tipo: " + tarjeta.getCardType() + " con el metodo de pago: " + paymentMethod.doPayment());
        System.out.println("Un esquema de pago: " + scheme.getpaymentScheme());

    }

    private static void probarSimpleFactory() {
        Payment payment = PaymentFactory.buildPayment(TypePayment.GOOGLEPAY);
        payment.doPayment();

        Payment paymentSpei = PaymentFactory.buildPayment(TypePayment.SPEI);
        paymentSpei.doPayment();
    }
}